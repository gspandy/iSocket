...
public class ClientAuthFilter extends BaseFilter {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ClientAuthFilter.class);
	...
	public NextAction handleRead(FilterChainContext ctx) throws IOException {
		...
		final ServiceResponse serviceResponse = (ServiceResponse) ctx
				.getMessage();
		...
		if (serviceResponse.getServiceId().equals(SocketKeys.SERVICE_ID_AUTH)) {
			...
			if (serviceResponse.getResponseCode() == SocketKeys.RESPONSE_CODE_SUCCESS) {
				...
				synchronized (connection) {
					// store id in the map
					ConnectionAuthInfo info = authenticatedConnections
							.get(connection);
					if (info == null) {
						serviceResponse
								.setResponseCode(SocketKeys.RESPONSE_CODE_AUTHENTICATIONERROR);
						serviceResponse
								.setResponseMessage(SocketKeys.MESSAGE_AUTH_DELETED);
					}
				...
				logger.error(String.format(
						SocketKeys.MESSAGE_SERVICE_ERROR,
						new Object[] { serviceResponse.getId(),
			...
			// if it's authentication response - we don't pass processing to a
			// next filter in a chain.
			...
			if (checkAuth(connection, sessionId)) {
				...
				serviceResponse
						.setResponseCode(SocketKeys.RESPONSE_CODE_AUTHENTICATIONERROR);
				serviceResponse
						.setResponseMessage(SocketKeys.MESSAGE_AUTH_ERROR);
	...
	public void puttConnectionAuthInfoIfAbsent(Connection connection,
	...
	public NextAction handleWrite(final FilterChainContext ctx)
		...
		if (authInfo == null) {
			...
			final ConnectionAuthInfo existingInfo = authenticatedConnections
					.putIfAbsent(connection, authInfo);
			if (existingInfo == null) {
				// it's the first message for this client - we need to start
		...
		if (authInfo.getState() == 0) {// un authenticate
			...
			// it might be a sign, that authentication has been completed on
			// another thread
			...
			synchronized (connection) {
				...
				if ((authInfo.getState() == 0)) {
					if (authInfo.sessionId == null) {
						// Authentication hs been started by another thread, but
			...
			throw new IllegalArgumentException(String.format(
					"auth info state:%s is error.", authInfo.getState()));
		}

		// Authentication has been completed - add "auth-id" header and pass the
		// message to a next filter in chain.
		sourceRequst.setSessionId(authInfo.getSessionId());
		return ctx.getInvokeAction();
	}

	private NextAction handleAuthFail(FilterChainContext ctx,
			ServiceRequest sourceRequest, String errMsg, Object... objects)
			throws IOException {
	...
	private boolean checkAuth(Connection connection, String sessionId) {
		...
		final ConnectionAuthInfo registeredId = authenticatedConnections
				.get(connection);
