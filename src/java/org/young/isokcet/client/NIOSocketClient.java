...
import org.young.isokcet.filter.ClientAuthFilter;
import org.young.isokcet.filter.ClientFutureFilter;
import org.young.isokcet.filter.ClientSSLFilter;
import org.young.isokcet.filter.ClientStringDataFilter;
...
public class NIOSocketClient {
    ...
    private void init() {
        ...
        if (StringUtils.isNotEmpty(this.userName) && StringUtils.isNotEmpty(this.password)) {
            filterChainBuilder.add(new ClientAuthFilter(this.userName, this.password));
        }
     ...
     * 当连接处于空闲状态(连接的两端没有互相传送数据) 超过了 2 小时时, 本地的TCP 实现
     * 会发送一个数据包给远程的 Socket. 如果远程Socket 没有发回响应, TCP实现就会持续
     * 尝试 11 分钟, 直到接收到响应为止. 如果在 12 分钟内未收到响应, TCP 实现就会自动
     * 关闭本地Socket, 断开连接. 在不同的网络平台上, TCP实现尝试与远程Socket 对话的时限有所差别.
     * </p>
     * @param
     * @return
     * @throws
     ...
     * 
     * <p>
     * 描述:那么执行Socket 的 close() 方法, 该方法不会立即返回, 而是进入阻塞状态. 同时, 底层的 Socket 会尝试发送剩余的数据. 只有满足以下两个条件之一, close() 方法才返回:
               ⑴ 底层的 Socket 已经发送完所有的剩余数据;
               ⑵ 尽管底层的 Socket 还没有发送完所有的剩余数据, 但已经阻塞了 x 秒(注意这里是秒, 而非毫秒), close() 方法的阻塞时间超过 3600 秒, 也会返回, 剩余未发送的数据被丢弃.
     ...
     * 
     * <p>
     * 描述:默认情况下, 发送数据采用Negale 算法. Negale 算法是指发送方发送的数据不会立即发出,
    ...
    public void connect(final String host, final int port) throws IOException {
        ...
        try {
            ...
            if (isNeedSSL()) {
                HandshakeStatus handshakeStatus = clientSSLFilter.getHandshakeStatus(10, TimeUnit.SECONDS);
                if (handshakeStatus == null || handshakeStatus != SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
                    throw new IOException(String.format("SSL handshake status:%s error!", handshakeStatus));
                }
            }
    ...
    //    public HandshakeStatus getSSLHandshakeStatus(long timeout, TimeUnit unit) throws IOException {
    //        try {
    ...
    public ServiceResponse getServiceResponse(long timeout, TimeUnit unit) throws IOException {
        try {
            ServiceResponse r = clientFutureFilter.getServiceResponse(timeout, unit);
            return r;
        } catch (IOException e) {
            logger.error("getServiceResponse error.", e);
            throw e;
    ...
    public void write(ServiceRequest request) throws IOException {
        try {
            SafeFutureImpl<ServiceResponse> resultFuture = SafeFutureImpl.create();
            clientFutureFilter.setFuture(resultFuture);
            GrizzlyFuture<WriteResult<ServiceRequest, SocketAddress>> write = connection.write(request);
        } catch (IOException e) {
    ...
    private SSLEngineConfigurator initializeSSL() {
        ...
        URL cacertsUrl = cl.getResource(getTrustStoreFile());//getTrustStoreFile() serverstore.jks
        if (cacertsUrl != null) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //final FutureImpl<ServiceResponse> resultMessageFuture = SafeFutureImpl.create();
